...
function navLinksFor(valid: Valid | null): NavLink[] {
  ...
  switch (valid.role) {
    ...
    case Role.Profissional:  return [
      { label: 'Dados Profissionais',icon: 'pi pi-briefcase',     route: `profissionais/${id}/dados-profissionais` },
      { label: 'Eventos',            icon: 'pi pi-calendar-plus', route: `profissionais/${id}/eventos` },
    ];
    case Role.Homecare:      return [
      { label: 'Histórico',          icon: 'pi pi-history',       route: `homecares/${id}/tratamento/preview` },
      { label: 'Tratamentos',        icon: 'pi pi-heart',         route: `homecares/${id}/tratamento/em-andamento` },
    ...
    case Role.PlanoSaude:    return [
      ...
      { label: 'Cadastrar Filial',   icon: 'pi pi-plus-circle',   route: `planos-saude/${id}/cadastro/filial` },
    ];
    ...
    case Role.Root:          return [
      ...
      { label: 'Cadastrar Evento',   icon: 'pi pi-envelope',      route: 'admin/eventos/cadastro' },
    ];
    default: return [];
  }
}
