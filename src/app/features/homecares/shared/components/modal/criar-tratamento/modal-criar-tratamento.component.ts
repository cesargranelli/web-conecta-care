...
import { PacienteService } from 'src/app/features/homecares/services/paciente.service';
...
export class ModalCriarTratamentoComponent implements OnInit {
  ...
  private redirecionamento(tratamentoEmAberto: TratamentoAberto) {
    ...
    if (tratamentoEmAberto) {
      ...
      this.router.navigate([`../medical-record/${tratamentoEmAberto?.paciente.id}`], { relativeTo: this.activatedRoute });
    } else {
      Swal.fire({
        position: 'center',
        icon: 'info',
