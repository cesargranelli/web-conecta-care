...
export class ProfissionalService {
  ...
  getDados(id: number): Observable<Profissional> {
    return this._http.get(${this.endpoint}/).pipe(map((p: Profissional) => p));
  }
  ...
  listarEventos(id: number): Observable<any> {
    return this._http.get<any>(${this.endpoint}//eventos, { observe: 'response' });
  }

  confirmarEvento(idProfissional: number, idEvento: number): Observable<any> {
    return this._http.put<any>(${this.endpoint}//events/, { observe: 'response' });
  }

  rejeitarEvento(idProfissional: number, idEvento: number): Observable<any> {
    return this._http.delete<any>(${this.endpoint}//events/, { observe: 'response' });
  }

  pesquisarPorCpf(cpf: string): Observable<Profissional> {
    return this._http.get(${this.endpoint}/cpf/).pipe(
      map((p: any) => p),
